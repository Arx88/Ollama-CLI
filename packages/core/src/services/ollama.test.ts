...
describe('OllamaClient', () => {
  ...
  describe('listModels', () => {
    it('should fetch and return a list of models', async () => {
      const mockModels = {
        models: [
          { name: 'llama2:latest', modified_at: '2023-10-26T14:00:00Z', size: 12345 },
  ...
  describe('showModelDetails', () => {
    ...
    it('should throw an error if showing model details fails', async () => {
      ...
      await expect(ollamaClient.showModelDetails('nonexistent-model')).rejects.toThrow(
  ...
  describe('generate', () => {
    ...
    it('should handle streaming generate request', async () => {
      const mockStreamChunks = [
        ...
        { model: 'test-model', response: 'World!', done: true, context: [1,2,3] },
      ];
    ...
    it('should throw an error if generate API request fails', async () => {
      ...
      await expect(ollamaClient.generate({ ...generateParams, stream: false })).rejects.toThrow(
        'Ollama API request failed: 404 Not Found - Model not found',
      );
    });
     it('should correctly parse stream with remaining buffer content', async () => {
      const mockStreamChunks = [
        { model: 'test-model', response: 'Final chunk', done: true, context: [1,2,3] },
      ];
